...
# Environment
# -------------------------------------------------------
resource "confluent_environment" "fraud_detect_env" {
...
resource "confluent_schema_registry_cluster" "simple_sr_cluster" {
    ...
    environment {
        id = confluent_environment.fraud_detect_env.id 
    }
    region {
...
resource "confluent_service_account" "app-ksql" {
  display_name = "svc-acct-app-ksql"
...
resource "confluent_role_binding" "app_manager_environment_admin" {
    ...
    crn_pattern = confluent_environment.fraud_detect_env.resource_name
}
